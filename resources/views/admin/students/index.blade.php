...
@section('content')
    ...
    <div class="page-header d-flex align-items-start justify-content-between flex-wrap gap-3">
        <div>
            <h1 class="page-title">{{ __('messages.students_title') }}</h1>
            <p class="page-sub">{{ __('messages.manage_students_desc') }}</p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            <a href="{{ route('admin.students.create') }}" class="btn-primary-sm"><i class="bi bi-plus-circle"></i>
                {{ __('messages.add_student') }}</a>
        </div>
    </div>
                ...
                <tbody>
                    @forelse($students as $student)
                        <tr>
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    @else
                                        <div class="avatar avatar-sm" style="background:#f5f3ff;color:#7c3aed">
                                            {{ strtoupper(substr($student->name, 0, 1)) }}</div>
                                    @endif
                                    <div>
                            ...
                            <td><span
                                    class="pill {{ $student->is_active ? 'pill-success' : 'pill-neutral' }}">{{ $student->is_active ? __('messages.Active') : __('messages.Inactive') }}</span>
                            </td>
                            <td style="color:var(--muted)">{{ $student->created_at->format('M d, Y') }}</td>
                            <td>
                                <div class="d-flex gap-1">
                                    <a href="{{ route('admin.students.show', $student->id) }}" class="btn-outline-sm"
                                        style="padding:4px 8px"><i class="bi bi-eye"></i></a>
                                    <a href="{{ route('admin.students.edit', $student->id) }}" class="btn-outline-sm"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-4" style="color:var(--muted)">
                                {{ __('messages.no_students_found') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
